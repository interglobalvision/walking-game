...
for(var temp,index,counter=array.length;counter>0;)index=Math.floor(Math.random()*counter),counter--,temp=array[counter],array[counter]=array[index],array[index]=temp;return array}},Utilities.Number={getRandomInt:function(min,max){return Math.floor(Math.random()*(max-min+1))+min}},Utilities.Word={adjs:[],nouns:[],init:function(adjsList,nounList){var _this=this;_this.adjs=adjsList,_this.nouns=nounList},/**
   ...
   */
getWord:function(kind,indefinite,capitalize){var _this=this,list="adj"==kind?_this.adjs:_this.nouns,word=list[Math.floor(Math.random()*list.length)];return capitalize&&(word=word.charAt(0).toUpperCase()+word.slice(1)),indefinite&&(word=_this.isVowel(word[0])?"an "+word:"a "+word),word},getAdj:function(indefinite,capitalize){var _this=this;return _this.getWord("adj",indefinite,capitalize)},getNoun:function(indefinite,capitalize){var _this=this;return _this.getWord("noun",indefinite,capitalize)},isVowel:function(character){return/[aeiouAEIOU]/.test(character)}},Utilities.Word.init(Adjs,Nouns),WalkingVariables={fadeSpeed:777,typeSpeed:33};
