...
_this.stop())},updateOrientation:function(orientation){var _this=this,northOrientation=-1*orientation,compensationAngle=_this.getAngle(_this.reference,_this.position,_this.destiny);
...
unsupportedGPS:function(){console.log("Fuck. No GPS"),alert("Sorry "+Game.getUsername()+", but your device does not support GPS or you have denied the app access to your location. You can't go walking if I don't know where you are. Open the Walking Game settings on your device and allow location access!!")}},Game={minigames:["tippyswitch","math","supertap","reset","colorsnap","vibeystopper","jankenpon","worldtraveler","medit8"],worlds:["Desert","City","Arctic","Jungle"],gameAttempts:2,shareTitle:function(score){return"WOOAAAAHH! U HAVE AN AWESOME SCORe 0F "+score+" POIIINTSSS BRAAAHHH"},shareSubject:"Subject: I did this on Walking Game. The most tiring phone game ever made",shareUrl:"http://interglobal.vision/",
// USER
createUser:function(username,callback){var _this=this;window.localStorage.setItem("username",username),window.localStorage.setItem("points",0),window.localStorage.setItem("gems",0),window.localStorage.setItem("progress",0),window.localStorage.setItem("distance",0),window.localStorage.setItem("loops",0),window.localStorage.setItem("world",0),window.localStorage.setItem("rank",_this.newRank()),_this.setupLoop(),callback()},getUsername:function(){return window.localStorage.getItem("username")},
// GAME STATE
setupLoop:function(){var _this=this;console.log("Setting up loop"),_this.setProgress(0),_this.setLoopOrder(Utilities.Misc.shuffleArray(_this.minigames))},getProgress:function(){var progress=parseInt(window.localStorage.getItem("progress"));return(null===progress||isNaN(progress))&&(progress=0),progress},setProgress:function(progress){window.localStorage.setItem("progress",progress)},getProgressPercent:function(){var _this=this,currentProgress=_this.getProgress();return currentProgress/this.minigames.length},setTotalDistance:function(newDistance){var _this=this,oldDistance=_this.getTotalDistance(),newDistance=parseFloat(newDistance);window.localStorage.setItem("distance",oldDistance+newDistance)},getTotalDistance:function(){return window.localStorage.getItem("distance")?parseFloat(window.localStorage.getItem("distance")):0},getTotalDistanceString:function(){var _this=this,totalDistance=_this.getTotalDistance(),distances=[[.0046,"anacondas"],[.025,"blue whales"],[.828,"Burj Khalifas"],[.006,"elephant trunks"],[.00206,"Ebenezer Places"],[385000.6,"Lunar distances"],[40075,"Earth circumferences"]],randomDistance=distances[Utilities.Number.getRandomInt(0,distances.length-1)],calcDistance=totalDistance/randomDistance[0];return calcDistance>1&&(calcDistance=calcDistance.toFixed(3)),calcDistance+" "+randomDistance[1]},getLoops:function(){var loops=parseInt(window.localStorage.getItem("loops"));return(null===loops||isNaN(loops))&&(loops=0),loops},setLoops:function(loops){window.localStorage.setItem("loops",loops)},setLoopOrder:function(loopOrder){window.localStorage.setItem("loopOrder",loopOrder)},getLoopOrder:function(){var loopOrder=window.localStorage.getItem("loopOrder");return loopOrder?loopOrder.split(","):[]},modifyDifficulty:function(difficulty){var _this=this,loops=_this.getLoops(),modifier=loops+1;return modifier*difficulty},nextMinigame:function(){var _this=this,currentProgress=_this.getProgress(),gameOrder=_this.getLoopOrder();console.log("Loading next minigame"),console.log("Current progress index",currentProgress),console.log("Game to load",gameOrder[currentProgress]),Router.go("/games/"+gameOrder[currentProgress]+"/")},finishLoop:function(){var _this=this,currentLoops=_this.getLoops();_this.getWorld();console.log("Finished loop"),
...
_this.setLoops(currentLoops+1),_this.nextWorld(),_this.setRank(),console.log("Loops so far",currentLoops),_this.setupLoop(),Router.go("/scenes/levelup/")},
// WORLD
...
// RANK
setRank:function(){var _this=this;window.localStorage.setItem("rank",_this.newRank())},newRank:function(){return Utilities.Word.getAdj(!0,!0)+" "+Utilities.Word.getNoun(!1,!0)},getRank:function(){return window.localStorage.getItem("rank")},
// MINI GAME
...
receivedEvent:function(id){console.log("Received Event: "+id),$("#game-username").html(Game.getUsername()),$("#game-points").html(Game.getPoints()),$("#game-gems").html(Game.getGems()),$("#game-progress").html(Game.getProgressPercent())}};app.initialize(),Utilities={},Menu={$blackout:$("#blackout"),$menu:$(".menu"),$menuButton:$("#map-menu-button"),$menuPoints:$("#menu-points"),$menuDistance:$("#menu-distance"),$menuRank:$("#menu-rank"),$menuWorld:$("#menu-world"),$buttonShare:$('[data-ref="menu-share"]'),$buttonOpenSub:$(".open-sub-menu"),$buttonCloseSub:$(".close-sub-menu"),howtoGreeting:"<p>dear "+Game.getUsername()+",</p>",resetDestinyDialog:["What? You lost? Pshh...","Ok, I'll set a new walking goal for you, "+Game.getUsername()+"...","...and imma take some points off your score!"],$resetDestiny:$('[data-ref="menu-reset-destiny"]'),
// Dev controls
$devMenu:$('[data-ref="dev-menu"]'),$devEnd:$('[data-ref="dev-end-map"]'),toggleMenu:function(){var _this=this;_this.closeSubMenu(),_this.$menu.toggle("fast")},init:function(){var _this=this;_this.$menuPoints.html(Game.getPoints()),_this.$menuDistance.html(Game.getTotalDistanceString()),_this.$menuWorld.html(Game.getWorldName()),_this.$menuRank.html(Game.getRank()),$(".howto-text").prepend(_this.howtoGreeting),
// Toggle menu
_this.$menuButton.on("click",function(){_this.toggleMenu()}),
// Share
_this.$buttonShare.on("click",function(event){event.preventDefault(),Game.shareWithOptions()}),
// Open sub menu
_this.$buttonOpenSub.on("click",function(event){event.preventDefault(),_this.openSubMenu($(this).attr("data-ref"))}),
// Close sub menu
_this.$buttonCloseSub.on("click",function(event){event.preventDefault(),_this.closeSubMenu()}),
// Reset destiny
_this.$resetDestiny.on("click",function(event){event.preventDefault(),_this.toggleMenu(),Utilities.Dialog.read(_this.resetDestinyDialog,function(){Compass.resetDestiny(function(){_this.$menuPoints.html(Game.getPoints())})})}),
// Dev control click events
_this.$devMenu.on("click",function(event){event.preventDefault(),Router.go("/pages/dev/")}),_this.$devEnd.on("click",function(event){event.preventDefault(),Compass.stop()})},openSubMenu:function(menu){$("#menu-main").addClass("show-sub-menu"),$("#"+menu).addClass("show-sub-menu")},closeSubMenu:function(){$("#menu-main").removeClass("show-sub-menu"),$(".sub-menu").removeClass("show-sub-menu")}},Router={init:function(){var _this=this,regex=/(.+?(?:www))/;_this.basePath=regex.exec(window.location.href),"browser"===window.cordova.platformId?_this.isBrowser=!0:_this.isBrowser=!1},go:function(url){var _this=this;_this.isBrowser?window.location=url:window.location=_this.basePath[0]+url+"index.html"}},Router.init(),Utilities.Color={isNeighborColor:function(color1,color2,tolerance){return void 0==tolerance&&(tolerance=32),Math.abs(color1[0]-color2[0])<=tolerance&&Math.abs(color1[1]-color2[1])<=tolerance&&Math.abs(color1[2]-color2[2])<=tolerance},hslToRgb:function(h,s,l){var r,g,b;if(0==s)r=g=b=l;else{var hue2rgb=function(p,q,t){return 0>t&&(t+=1),t>1&&(t-=1),1/6>t?p+6*(q-p)*t:.5>t?q:2/3>t?p+(q-p)*(2/3-t)*6:p},q=.5>l?l*(1+s):l+s-l*s,p=2*l-q;r=hue2rgb(p,q,h+1/3),g=hue2rgb(p,q,h),b=hue2rgb(p,q,h-1/3)}return[Math.round(255*r),Math.round(255*g),Math.round(255*b)]}},Utilities.Dialog={$target:$(".text-box-dialog"),$parent:$("#dialog"),$skip:void 0,interval:33,arrayIndex:0,lineIndex:0,lineTimer:0,outputText:"",inProgress:!1,read:function(dialogArray,callback){var _this=this;_this.$parent=$("#dialog"),_this.$target=$(".text-box-dialog"),_this.dialogArray=dialogArray,_this.arrayIndex=0,_this.$parent.show(),_this.inProgress=!0,_this.$parent.append('<div id="dialog-skip"></div>'),_this.$skip=$("#dialog-skip"),_this.$skip.off("click.dialogRead").on({"click.dialogRead":function(){_this.lineTimer>0?_this.skipLine():_this.arrayIndex===_this.dialogArray.length-1?_this.finish(callback):(_this.arrayIndex++,_this.readLine())}}),_this.readLine()},readLine:function(){var _this=this,dialogLine=_this.dialogArray[_this.arrayIndex];_this.lineIndex=0,_this.$target.html(""),_this.outputText="",_this.lineTimer=setInterval(function(){_this.lineIndex<dialogLine.length?(_this.outputText+=dialogLine[_this.lineIndex],_this.$target[0].innerHTML=_this.outputText,_this.lineIndex++):(_this.clearLineInterval(),_this.$target.append('<a class="text-box-next">&rarr;</a>'))},_this.interval)},clearLineInterval:function(){var _this=this;clearInterval(_this.lineTimer),_this.lineTimer=0},skipLine:function(){var _this=this;_this.clearLineInterval(),_this.$target.html(_this.dialogArray[_this.arrayIndex]),_this.$target.append('<a class="text-box-next">&rarr;</a>')},finish:function(callback){var _this=this;_this.$parent.hide(),_this.inProgress=!1,_this.$target.html(""),_this.$skip.remove(),callback&&callback()}},Utilities.Misc={shuffleArray:function(array){
// While there are elements in the array
for(var temp,index,counter=array.length;counter>0;)index=Math.floor(Math.random()*counter),counter--,temp=array[counter],array[counter]=array[index],array[index]=temp;return array},vibrate:function(otherDuration){var duration=1e3;otherDuration&&(duration=otherDuration),navigator.vibrate?navigator.vibrate(duration):console.log("no vibrate support")}},Utilities.Number={getRandomInt:function(min,max){return Math.floor(Math.random()*(max-min+1))+min},roundFloat:function(number){
